...
# data = load_dataset(file_path)
processed_data = load_dataset(file_path)

#processed_data, scaling_factors = preprocess_data(data.copy())   

# print(processed_data.to_string(index=False))

# Grouping by 'product_id' and selecting the first row (day 1) for each group
# day_1_records = processed_data[processed_data['day'] == 1]
